...
def test(
    ...
    for sample_batch in loader:
        ...
        targets = {'gt_matrix': gt_matrix , 'h_gt' : h_gt}
...
def train(
    ...
    max_norm=0., print_freq=10000., tb_logger=None
):
    model.train()
    criterion.train()
...
def main(args):
    ...
    save_path_all = "artifacts/nyu_logs_new_all2"
    ...
    for epoch in range(args.train_epoch):
        ...
        loss1 = log_stats['test_losses']
        loss2 = log_stats['test_fine_loss']
        torch.save({'model': model_without_ddp.state_dict()},
                   f'{save_path_all}/{epoch}_model_RGBD_normalize_best_{loss1:.1f}_{loss2:.1f}.pth')
        if log_stats['test_losses'] < best_loss:
            best_loss = log_stats['test_losses']
            fine_loss = log_stats['test_fine_loss']
            torch.save({'model': model_without_ddp.state_dict()}, f'{save_path}/{epoch}_model_RGBD_normalize_best_{best_loss:.1f}_{fine_loss:.1f}.pth')
        with open(f'{save_path}/train_RGBD_normalize.log', 'a') as f:
