...
def test(
    ...
    for sample_batch in logger.log_every(loader, print_freq, header):
        ...
        cv2.imwrite(f"nyu_results/{IM_POS}.jpg", out2)
        IM_POS += 1
...
def main(args):
    ...
    res = {}
    model_names = os.listdir("/home/ly/ML/trr_work/SemanticMatch/train_nyu_mamba/artifacts/nyu_logs_new_all_3/")
    for model_name in [x for x in model_names if '5_model_RGBD_normalize_best_' in x]:
        state_dict = torch.load(f"/home/ly/ML/trr_work/SemanticMatch/train_nyu_mamba/artifacts/nyu_logs_new_all_3"
                                f"/{model_name}", map_location='cpu')
