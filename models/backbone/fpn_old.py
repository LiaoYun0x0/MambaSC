...
class FPN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_encoding_16 = PositionEncodingSine(
            config['d_model_16'],
            temp_bug_fix=config['temp_bug_fix'])
        self.mamba2_block_4_1 = Mamba2(Mamba2Config(d_model=192, n_layer=2), device='cuda')
        self.mamba2_block_8_1 = Mamba2(Mamba2Config(d_model=256, n_layer=4), device='cuda')
        self.mamba2_block_16 = Mamba2(Mamba2Config(d_model=64, n_layer=2), device='cuda')
        self.mamba2_block_4_2 = Mamba2(Mamba2Config(d_model=192, n_layer=2), device='cuda')
        self.mamba2_block_8_2 = Mamba2(Mamba2Config(d_model=256, n_layer=4), device='cuda')
        # 每种卷积的输出通道分配1
        split_channels = config['d_model_8'] // 4
        in_channels = config['d_model_8']
        out_channels = config['d_model_8']
        # 不同感受野的卷积层
        self.conv1x1 = nn.Sequential(
            nn.Conv2d(in_channels, split_channels, kernel_size=1, padding=0),
            nn.BatchNorm2d(split_channels),
            nn.ReLU(inplace=True)
        )
        self.conv3x3 = nn.Sequential(
            nn.Conv2d(in_channels, split_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(split_channels),
            nn.ReLU(inplace=True)
        )
        self.conv5x5 = nn.Sequential(
            nn.Conv2d(in_channels, split_channels, kernel_size=5, padding=2),
            nn.BatchNorm2d(split_channels),
            nn.ReLU(inplace=True)
        )
        self.conv7x7 = nn.Sequential(
            nn.Conv2d(in_channels, split_channels, kernel_size=7, padding=3),
            nn.BatchNorm2d(split_channels),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        x3, h = self.mamba2_block_8_1(x3)
        x3 = rearrange(x3, 'n (h w) c -> n c h w', h=data['x3_h'], w=data['x3_w'])
        # 不同感受野的卷积特征
        x3_1 = self.conv1x1(x3)  # 1x1 卷积
        x3_3 = self.conv3x3(x3)  # 3x3 卷积
        x3_5 = self.conv5x5(x3)  # 5x5 卷积
        x3_7 = self.conv7x7(x3)  # 7x7 卷积
        # 拼接所有特征图
        fused_feat = torch.cat((x3_1, x3_3, x3_5, x3_7), dim=1)  # 按通道维度拼接
        # 通过轻量化的 1x1 卷积融合通道
        x3 = self.light_fusion(fused_feat)
        x4 = self.stage4(x3)  # 1/16
        x4_res = x4
        data['x4_h'] = x4.size(2)
        data['x4_w'] = x4.size(3)
        x4 = self.pos_encoding_16(x4)
        _H = (data['x4_h'] + 7) // 8 * 8  # 8 的 倍数
        _W = (data['x4_w'] + 7) // 8 * 8  # 8 的 倍数
        pad_info = _pad8(x4)
        _x = pad(x4, *pad_info)
        data['x4_h_'] = _x.size(2)
        data['x4_w_'] = _x.size(3)
        _x = rearrange(_x, 'b c h w -> b (h w) c')
        # 首先将通道维度拆分成8个部分，每个部分有64个通道
        b, _, c = _x.size()
        assert c == 512, "_x 的通道数必须为512"
        split_size = c // 8  # 64

        # 使用 torch.split 来拆分通道维度
        _x_splits = torch.split(_x, split_size, dim=2)  # 在通道维度上拆分
        ...
        for _x_split in _x_splits:
            ...
            output, h = self.mamba2_block_16(_x_split)  # 注意这里返回了两个值，我们只需要output进行拼接
            outputs.append(output)

        x4 = torch.cat(outputs, dim=2)  # 在通道维度上拼接
        _y = rearrange(x4, 'b (h w) c -> b c h w', h=data['x4_h_'], w=data['x4_w_'])
        x4 = unpad(_y, *pad_info)
        x4_out = self.layer4_outconv(x4) # 1/16

        x4_out_2x = F.interpolate(x4_out, scale_factor=2., mode='bilinear', align_corners=True)  # 1/8 512
        x3_out = self.layer3_outconv(x3_res)  # 1/8 512
        x3_out = self.layer3_outconv2(x3_out+x4_out_2x)  # 1/8
        x3_out = rearrange(self.pos_encoding_8(x3_out), 'n c h w -> n (h w) c')
        x3_out, h = self.mamba2_block_8_2(x3_out)
        x3_out = rearrange(x3_out, 'n (h w) c -> n c h w', h=data['x3_h'], w=data['x3_w'])
        ...
        x1_out = rearrange(self.pos_encoding_2(x1_out), 'n c h w -> n (h w) c')
        # x1_out, h = self.mamba2_block_2_2(x1_out)
        x1_out = rearrange(x1_out, 'n (h w) c -> n c h w', h=data['x1_h'], w=data['x1_w'])
