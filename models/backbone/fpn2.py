...
class FPN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mamba2_block_8_cut = Mamba2(Mamba2Config(d_model=64, n_layer=2), device='cuda')
    ...
    def forward(self, x):
        ...
        b, _, c = x3.size()
        assert c == 256, "_x 的通道数必须为512"
        split_size = c // 4  # 64

        # 使用 torch.split 来拆分通道维度
        _x_splits = torch.split(x3, split_size, dim=2)  # 在通道维度上拆分
        ...
        for _x_split in _x_splits:
            ...
            output, h = self.mamba2_block_8_cut(_x_split)  # 注意这里返回了两个值，我们只需要output进行拼接
            outputs.append(output)

        x3 = torch.cat(outputs, dim=2)  # 在通道维度上拼接
        x3 = x3 * 0.1 + x3_b * 0.9
        x3 = rearrange(x3, 'b (h w) c -> b c h w', h=data['x3_h'], w=data['x3_w'])
        x3_out = self.layer3_outconv2(x3_res+x3)  # 1/8
        ...
        x2_out = x2_out * 0.1 + x2_out_a * 0.9
